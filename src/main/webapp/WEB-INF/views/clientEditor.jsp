...
<html>
    <head>
		<title>Edition des clients</title>
	</head>
	...
	<body>
		...
		<form method="POST"> <%-- L'action par défaut est de revenir à l'URL du contrôleur --%>
			<input name="code" placeholder="Entrer un code"><br>
			...
			<input name="adresse" placeholder="Rentrer adresse"><br>
			<ul> <%-- On montre les erreurs de saisie éventuelles --%>
				<c:forEach var="error" items="${validationErrors.getErrors('adresse')}">
					<li><span style="color: red;">${mvc.encoders.html(error.message)}</span></li>
				</c:forEach>
			</ul>
			<input type="submit" value="ajouter un nouveau client">
		</form>
		...
		<table border='1'>
			<tr><th>Code</th><th>Adresse</th></tr>
